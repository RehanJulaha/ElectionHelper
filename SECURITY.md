...
- **Cloud Functions** validate input length; callables use **`enforceAppCheck: true`** (see `functions/src/index.ts`).
- **Cloud Build** (`cloudbuild.yaml`) reads the Firebase CI token from **Secret Manager** (`firebase-ci-token` → `FIREBASE_CI_TOKEN`); do not commit deploy tokens.
...
5. The SPA registers App Check via `@angular/fire` (`provideAppCheck` + `ReCaptchaEnterpriseProvider`) when `appCheckSiteKey` is non-empty (`src/app/app.config.ts`, `buildFirebaseClientProviders`).
