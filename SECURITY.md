...
- **CSP** and security headers configured in `firebase.json` for Hosting (including domains required by **App Check** / reCAPTCHA Enterprise and Firebase Performance). `style-src` omits `'unsafe-inline'`. The production Angular build disables **critical CSS / font inlining** (`optimization.styles.inlineCritical: false`, `optimization.fonts.inline: false` in `angular.json`) so `index.html` is not given inline `<style>` blocks or `onload="…"` handlers on stylesheet links (those were tripping `style-src` and `script-src`). Dynamic theme tokens use **`CSSStyleSheet` + `document.adoptedStyleSheets`** in `ThemeService` instead of `element.style.setProperty`, which would count as inline styles. If a dependency or Angular feature starts requiring inline styles again, either relax `style-src` for that release or add a **per-request nonce / SSR** pipeline.
